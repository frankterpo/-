...
class PinchedFingersRenderer {
public:
    PinchedFingersRenderer(int w = 70, int h = 34, int frameLimit = 120, bool live = false)
        : width(w),
          height(h),
    ...
    ~PinchedFingersRenderer() { showCursor(); }
...
private:
    ...
    std::vector<Vec3> vertices; // every 3 vertices define one triangle
    ...
    void loadMesh() {
        std::ifstream in("hand_mesh.csv");
        if (!in.is_open()) {
            std::cerr << "Could not open hand_mesh.csv.\n";
            running = false;
            return;
        }
        ...
        while (std::getline(in, line)) {
            ...
            Vec3 v{};
            if (std::sscanf(line.c_str(), "%f,%f,%f", &v.x, &v.y, &v.z) == 3) {
    ...
    void rasterTriangle(const ProjV& v0, const ProjV& v1, const ProjV& v2, float brightness) {
    ...
    void rasterizeMesh() {
        ...
        Vec3 lightDir = normalize({0.45f, -0.35f, 0.82f});
        size_t triCount = vertices.size() / 3;

        for (size_t t = 0; t < triCount; ++t) {
            const Vec3& a = vertices[t * 3 + 0];
            const Vec3& b = vertices[t * 3 + 1];
            const Vec3& c = vertices[t * 3 + 2];
            ...
            Vec3 fn = normalize(n * -1.0f); // facing normal

            float diffuse = std::max(0.0f, dot(fn, lightDir));
            float brightness = std::min(1.0f, 0.16f + 0.84f * diffuse);
            ...
            rasterTriangle(p0, p1, p2, brightness);
        }
...
int main(int argc, char** argv) {
    ...
    PinchedFingersRenderer renderer(70, 34, frameLimit, live);
    renderer.run();
    return 0;
}
